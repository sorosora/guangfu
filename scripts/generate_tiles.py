...
try:
    ...
    print("請執行：pip install pillow numpy requests boto3 urllib3")
    sys.exit(1)
...
class IntelligentTileGenerator:
    ...
    def get_changed_grids(self) -> List[str]:
        """獲取變更網格 - 極簡策略的核心"""
        try:
            result = self._redis_request('SMEMBERS', 'changed_grids')
    ...
    def read_changed_grids_states(self, changed_grid_ids: List[str]) -> Dict[str, int]:
        """只讀取變更網格的狀態 - 極簡策略的關鍵"""
        if not changed_grid_ids:
            return {}
        
        print(f"🔍 只讀取 {len(changed_grid_ids)} 個變更網格")
        grid_states = {}
        
        for grid_id in changed_grid_ids:
            try:
                result = self._redis_request('HGET', f'grid:{grid_id}', 'finalState')
    ...
    def get_latest_version(self) -> Optional[str]:
        ...
        try:
            # 從 Redis 獲取最新版本
            result = self._redis_request('GET', 'tile_version')
    ...
    def clear_changed_grids(self):
        """清除變更標記"""
        try:
            self._redis_request('DEL', 'changed_grids')
            print("🧹 變更標記已清除")
        except Exception as e:
            print(f"❌ 清除變更標記失敗: {e}")

    def generate_intelligent_tiles(self):
        """執行 OGC TMS 相容的智能增量圖磚生成"""
        print("🚀 === OGC TMS 智能增量圖磚生成 v3.0 ===")
        print(f"⏰ 開始時間: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        # 1. 獲取變更網格
        changed_grids = self.get_changed_grids()
        if not changed_grids:
            print("ℹ️ 沒有網格變更，跳過圖磚生成")
            print(f"⏱️ 執行時間: {time.time() - self.stats['start_time']:.2f} 秒")
            return
        
        print(f"📍 發現 {len(changed_grids)} 個變更網格")
        
        # 2. 獲取最新版本
        previous_version = self.get_latest_version()
        if previous_version:
        ...
        # 5. 只讀取變更網格狀態
        changed_states = self.read_changed_grids_states(changed_grids)
        ...
        # 10. 清除變更標記
        self.clear_changed_grids()
        
        # 11. 最終統計
        duration = time.time() - self.stats['start_time']
        
        print(f"\n🎉 === 智能增量圖磚生成完成 ===")
        print(f"✅ 成功處理 {success_count}/{len(affected_tiles)} 個圖磚")
        print(f"📁 複製未變更圖磚: {copied_count} 個")
        print(f"📥 下載現有圖磚: {self.stats['tiles_downloaded']} 個")
        print(f"⚡ Redis 請求總數: {self.stats['redis_requests']}")
        print(f"📦 上傳總量: {self.stats['bytes_uploaded']:,} bytes")
        print(f"⏱️ 執行時間: {duration:.2f} 秒")
        print(f"💰 月度預估: {8640 * duration / 60:.1f} 分鐘")
        print(f"🆔 版本號: {new_version}")
        print(f"🌐 符合 OGC TMS 標準")
        
        # 驗證預算符合性
        monthly_minutes = 8640 * duration / 60
